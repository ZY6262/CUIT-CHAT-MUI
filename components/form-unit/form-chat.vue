<template>
	
	<view class="cu-list menu-avatar">
		<view class="cu-item" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(item, index) in chatList" :key="index"
		 @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-' + index">
			<view class="cu-avatar round lg" :style="'background-image:url(' + item.avatar + ');'"></view>
			<view class="content">
				<view class="text-grey"> {{item.name}}</view>
				<view class="text-gray text-sm">
					<text class="cuIcon-infofill text-red  margin-right-xs"></text> {{item.desc}}</view>
			</view>
			<view class="action">
				<view class="text-grey text-xs">{{item.time}}</view>
				<view :class="['cu-tag round sm', item.badgeNum > 0 ? 'bg-red' : 'bg-white']"><text :style="item.badgeNum > 0 ? '' : 'color: white'">{{item.badgeNum}}</text></view>
			</view>
			<view class="move">
				<view class="bg-grey">置顶</view>
				<view class="bg-red">删除</view>
			</view>
		</view>
	</view>
</template>

<script>
	export default {
		props: {
			chatList: {
				type: Array,
				default: [{
					avatar: 'https://ossweb-img.qq.com/images/lol/img/champion/Taric.png',
					badgeNum: 1,
					time: '10:30',
					name: '昨日少年郎',
					desc: '正在抓紧开发'
				},
				{
					avatar: 'https://ossweb-img.qq.com/images/lol/img/champion/Taric.png',
					badgeNum: 1,
					time: '10:30',
					name: '昨日少年郎',
					desc: '正在抓紧开发'
				},
				{
					avatar: 'https://ossweb-img.qq.com/images/lol/img/champion/Taric.png',
					badgeNum: 0,
					time: '10:30',
					name: '昨日少年郎',
					desc: '正在抓紧开发'
				},
				{
					avatar: 'https://ossweb-img.qq.com/images/lol/img/champion/Taric.png',
					badgeNum: 1,
					time: '10:30',
					name: '昨日少年郎',
					desc: '正在抓紧开发'
				},
				{
					avatar: 'https://ossweb-img.qq.com/images/lol/img/champion/Taric.png',
					badgeNum: 1,
					time: '10:30',
					name: '昨日少年郎',
					desc: '正在抓紧开发'
				}]
			}
		},
		data() {
			return {
				listTouchStart: 0,
				listTouchDirection: null,
				modalName: null
			}
		},
		methods: {
			// ListTouch触摸开始
			ListTouchStart(e) {
				this.listTouchStart = e.touches[0].pageX
			},
			
			// ListTouch计算方向
			ListTouchMove(e) {
				this.listTouchDirection = e.touches[0].pageX - this.listTouchStart > 0 ? 'right' : 'left'
			},
			
			// ListTouch计算滚动
			ListTouchEnd(e) {
				if (this.listTouchDirection == 'left') {
					this.modalName = e.currentTarget.dataset.target
				} else {
					this.modalName = null
				}
				this.listTouchDirection = null
			}
		}
	}
</script>

<style scoped>
.page {
		height: 100Vh;
		width: 100vw;
	}

	.page.show {
		overflow: hidden;
	}

	.switch-sex::after {
		content: "\e716";
	}

	.switch-sex::before {
		content: "\e7a9";
	}

	.switch-music::after {
		content: "\e66a";
	}

	.switch-music::before {
		content: "\e6db";
	}
</style>
